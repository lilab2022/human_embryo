...
###
meta_addition<-read.csv("~/data/subtypes_mf_additinal_meta.csv",header=T)
load("~/data/mac.color2.rdata")

data<-read.csv("~/data/cleanumi_MC.csv",header=T,row.names = "X") # loading mc level umi-tab 

data<-data[,colnames(data)%in%meta_addition$MC]

###
excluded.genes<-read.csv("~/data/excludeall_gene.csv",header=T) ## loading forbidden genes
...
write.csv(template,"~/[data]/gene_reorder_fig3_suppl.csv")
...
ggsave(p,filename = "~/[data]/figS3A.pdf",width = 20, height = 10, units = "in", device='pdf')
...
differentiation<-read.csv("~/data/mf_prog_cytotrace.csv") # differentiation data was generated by cytotrace
...
ggsave("~/data/differentiation_boxplot2.pdf",p1,width = 12,height = 6,dpi = 300,units="in",device="pdf")
...
ggsave("~/data/differentiation_boxplot_byGroup.pdf",p3,width = 12,height = 6,dpi = 300,units="in",device="pdf")


#### proliferation

proliferation<-read.table("~/data/CellcycleScoring.csv") # cell cylce score calculated by the genes listed in supplementary table 7
...
ggsave("~/data/proliferation_boxplot.pdf",p1,width = 12,height = 6,dpi = 300,units="in",device="pdf")


# FigureS 3B Lower

meta_addition<-read.csv("~/data/subtypes_mf.csv",header=T)
meta_addition<-meta_addition[,c(1,3,4,7)]
colnames(meta_addition)[1]<-"mc"
...
ggsave("~/[folder]/proliferation_boxplot_byGroup.pdf",p3,width = 12,height = 6,dpi = 300,units="in",device="pdf")
...
scalar_plot<-function(data_prep2,A,B,A_1,B_1,genelist,A_color,B_color){
  ...
  p<-ggplot(data_prep2,aes(x = get(A),y = get(B),label = label))+
    ...
    geom_text_repel(data = data_prep2[data_prep2 != "",],max.overlaps = 100000)+theme_bw() + 
    
    labs(x = A_1, y = B_1, color = '',subtitle = paste("Correlation:",corr,sep="")) + 
  ...
  ggsave(p,filename = paste("~/[folder]/",A,"_VS_",B,"_scalar_plot.pdf",sep=""),width = 5, height = 5, units = "in", device='pdf')
...
load("[umi-tab]") # loading umi-tab at single-cell level, downloaded from the link in China National GeneBank, see manuscript
load("~/data/metatable_0929.Rdata")

#gene_counts<-gene_counts[rownames(gene_counts)%in%excluded_genes,]

mf_id<-read.csv("~/data/subtypes_mf_additinal_meta",header = T) # loading  macrophage meta information
...
#Figure 2S F
          ### S2F mac composition for each tissue (thymus removed)
          ...
          ### S2F tissue composition for each mac subtype 
